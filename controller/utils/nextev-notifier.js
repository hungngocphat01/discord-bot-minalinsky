...
function decideChannel(eventDate, today) {
    ...
    } else if (dayDiff <= 2.5 || process.env.RUNNING_ON_HEROKU == 0) {
...
async function notifier (client) {
    ...
    // Get and print next events
    const events = await Event.fetchNextEvents();
    logger(`Next events: ${events.length}`);
    logger(JSON.stringify(events));
    
    // If there are events, send them to noti channel
    if (events.length > 0) {
        ...
        const embeds = await Generator.generateNotifEmbeds(events, true);
